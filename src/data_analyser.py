...
class FingerDataAnalyser:
    ...
    def analyze_scores(self, df: pd.DataFrame, filename: str = "score_summary.md") -> None:
        ...
        if df_numeric.empty:
            lines.append("No numeric columns were found for analysis.")
            self._write_markdown(md_path, lines)
            return

        # Group numeric columns by metric suffix after the first underscore
        # Column names created by refomrat_df are in the form "<TEST>_<METRIC>"
        metrics: set[str] = set()
        for col in df_numeric.columns:
            if "_" in col:
                _, metric = col.split("_", 1)
                metrics.add(metric)

        # For each metric (e.g. correct_sequences, keystrokes, ratio) produce a separate table
        for metric in sorted(metrics):
            metric_cols = [c for c in df_numeric.columns if c.split("_", 1)[1] == metric]
            if not metric_cols:
                continue

            lines.extend([f"## Metric: {metric}", ""])

            # Compute descriptive summary for these columns
            df_metric = df_numeric[metric_cols]
            df_summary = self._score_summary_table(df_metric)

            # Build rows showing per-test summaries; extract test name from column prefix
            rows = []
            for column, row in df_summary.iterrows():
                # `column` here is the original column name (e.g. "FT1_correct_sequences")
                test_name = column.split("_", 1)[0] if "_" in column else column
                rows.append(
                    [
                        self._format_md_value(test_name),
            ...
            lines.extend(
                self._markdown_table(
                    ["Test", "Mean", "Std Dev", "Std Error", "CI Lower (95%)", "CI Upper (95%)", "T Critical", "Normality"],
                    rows,
                )
            )
            lines.append("")
    ...
    def analyze_by_timepoint(self, df: pd.DataFrame, filename: str = "timepoint_summary.md") -> Path:
        ...
        df_wide = self.refomrat_df(df)
        # Determine available metrics from FT1-FT4 columns (e.g. correct_sequences, keystrokes, ratio)
        metrics: set[str] = set()
        for col in df_wide.columns:
            if "_" not in col:
                continue
            prefix, metric = col.split("_", 1)
            if prefix.upper() in {"FT1", "FT2", "FT3", "FT4"}:
                metrics.add(metric)

        if not metrics:
            raise ValueError("Could not find FT1-FT4 metric columns for timepoint analysis")

        md_path = self.output_dir / filename
        lines = ["# Timepoint Summary", ""]

        # For each metric produce per-participant, overview and comparison results
        for metric in sorted(metrics):
            pretest_cols = [
                col
                for col in df_wide.columns
                if (col.startswith("FT1_") or col.startswith("FT2_")) and col.split("_", 1)[1] == metric
            ]
            posttest_cols = [
                col
                for col in df_wide.columns
                if (col.startswith("FT3_") or col.startswith("FT4_")) and col.split("_", 1)[1] == metric
            ]

            if not pretest_cols or not posttest_cols:
                # skip metric if either side missing
                continue
            ...
            lines.extend([f"## Metric: {metric}", "", "### Per Participant", ""])
            participant_rows = [
                [
            ...
            lines.extend(self._markdown_table(["Participant_ID", f"Pretest_Avg ({metric})", f"Posttest_Avg ({metric})"], participant_rows))
